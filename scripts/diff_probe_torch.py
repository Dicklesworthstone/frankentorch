...
# ── batch 5: reductions over inf/nan + comparison ──
def t1(v): return torch.tensor(v)
print("prod_0inf|" + fmt(torch.prod(t1([2.0, 0.0, INF])).reshape(1)))
print("sum_infninf|" + fmt(torch.sum(t1([INF, -INF])).reshape(1)))
print("nansum|" + fmt(torch.nansum(t1([1.0, NAN, 2.0])).reshape(1)))
print("nansum_allnan|" + fmt(torch.nansum(t1([NAN, NAN])).reshape(1)))
print("nanmean|" + fmt(torch.nanmean(t1([1.0, NAN, 3.0])).reshape(1)))
print("nanmean_allnan|" + fmt(torch.nanmean(t1([NAN, NAN])).reshape(1)))
print("var_n1_c1|" + fmt(torch.var(t1([5.0]), correction=1).reshape(1)))
print("var_n1_c0|" + fmt(torch.var(t1([5.0]), correction=0).reshape(1)))
print("std_n1_c1|" + fmt(torch.std(t1([5.0]), correction=1).reshape(1)))
print("amax_nan|" + fmt(torch.amax(t1([1.0, NAN, 3.0]), 0).reshape(1)))
print("amin_nan|" + fmt(torch.amin(t1([1.0, NAN, 3.0]), 0).reshape(1)))
print("cumprod_0inf|" + fmt(torch.cumprod(t1([1.0, 0.0, INF, 2.0]), 0)))
print("cumsum_infninf|" + fmt(torch.cumsum(t1([1.0, INF, -INF, 1.0]), 0)))
print("logsumexp_ninf|" + fmt(torch.logsumexp(t1([-INF, -INF]), 0).reshape(1)))
print("logsumexp_inf|" + fmt(torch.logsumexp(t1([INF, 1.0]), 0).reshape(1)))
print("norm2_inf|" + fmt(torch.norm(t1([INF, 1.0, 2.0]), 2.0).reshape(1)))
a = t1([NAN, 1.0, INF, -INF]); b = t1([NAN, 1.0, INF, -INF])
print("isclose_eqnanF|" + fmt(torch.isclose(a, b, 1e-5, 1e-8, False).double()))
print("isclose_eqnanT|" + fmt(torch.isclose(a, b, 1e-5, 1e-8, True).double()))

# ── batch 6: special-function accuracy ──
sp6 = torch.tensor([0.5, 1.0, 2.0, 3.0, 5.0, 8.0])
print("i0|" + fmt(torch.special.i0(sp6)))
print("i1|" + fmt(torch.special.i1(sp6)))
print("i0e|" + fmt(torch.special.i0e(sp6)))
print("i1e|" + fmt(torch.special.i1e(sp6)))
print("bessel_j0|" + fmt(torch.special.bessel_j0(sp6)))
print("bessel_j1|" + fmt(torch.special.bessel_j1(sp6)))
print("bessel_y0|" + fmt(torch.special.bessel_y0(sp6)))
print("bessel_k0|" + fmt(torch.special.modified_bessel_k0(sp6)))
print("erfcx|" + fmt(torch.special.erfcx(sp6)))
print("ndtr|" + fmt(torch.special.ndtr(torch.tensor([-3.0, -1.0, 0.0, 1.0, 2.0, 3.0]))))
print("ndtri|" + fmt(torch.special.ndtri(torch.tensor([0.01, 0.1, 0.25, 0.5, 0.9, 0.99]))))
