...
| `GW-002` | legacy oracle timeout/cancel/hang envelope | closed on 2026-05-10 via `frankentorch-6qmt`: FT-P2C-001 e2e matrix now emits a bounded `legacy_oracle_timeout_bounded` envelope and differential oracle failures classify timeout separately from generic unavailability | `frankentorch-6qmt` | keep timeout/hang kill-path evidence bound to `legacy_oracle` suite, `GW-002`, and `terminate_and_reap_child` |
| `GW-003` | runtime durability-ledger assertions in integrated conformance/e2e path | closed on 2026-05-10 via `frankentorch-ueqf`: FT-P2C-006 serialization conformance now bridges the latest `EvidenceKind::Durability` entry into `runtime_durability_summary` and publishes full `runtime_evidence` entries; `strict_serialization_expected_error_logs_runtime_durability_summary` asserts the summary and durability entry are present | `frankentorch-ueqf`, `bd-3v0.9`, `bd-3v0.10`, `bd-3v0.17.7` | keep runtime durability assertions bound to `runtime_durability_summary`, `runtime_evidence`, and `serialization/strict:checkpoint_incompatible_payload_top_level_array` |
