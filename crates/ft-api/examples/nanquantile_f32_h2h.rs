...
"#,
        ...
        println!(
            "parity q={q}: dtype={dt:?}(was ERROR) ft={fv:.7} torch={p32:.7} rel={rel:.2e} (tol<1e-6: {})",
            rel < 1e-6
        );
    }
    ...
    let tadd = || {
        ...
        for _ in 0..7 {
            ...
            let y = s.tensor_variable_f32(big.clone(), vec![n], false).unwrap();
            let ti = Instant::now();
    ...
    let tnq = || {
        ...
        for _ in 0..7 {
            ...
            let ti = Instant::now();
            let _ = s.tensor_nanquantile(x, 0.5);
            let e = ti.elapsed().as_secs_f64() * 1e3;
            if e < bst {
                bst = e;
            }
