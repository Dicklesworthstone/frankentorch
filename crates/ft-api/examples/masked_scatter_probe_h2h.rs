    ...
    let bench = || {
        ...
        for _ in 0..5 {
            ...
            let xi = s
                ...
                .unwrap();
            let mk = s
                .tensor_variable_f32(mask.clone(), vec![rows, cols], false)
                .unwrap();
            let sr = s
                .tensor_variable_f32(source.clone(), vec![rows, cols], false)
                .unwrap();
            let t = Instant::now();
...
"#
    ...
    println!(
        "masked_scatter f32 [{rows}x{cols}]: FT {ft:8.3}ms torch {pt:8.3}ms => {vrb} | dtype={dt:?} exact={exact}/{}",
        fv.len().min(tv.len())
    );
    Ok(())
}
