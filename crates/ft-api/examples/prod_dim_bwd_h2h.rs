...
// Head-to-head timing + grad fingerprint for the fused prod_dim backward
// (frankentorch-normp-bwd-fused). Values kept near 1.0 so the length-4096
// product does not overflow. Compare vs torch via the companion scratch script.
...
fn main() {
    ...
    let data: Vec<f64> = (0..n)
        .map(|i| 1.0 + (((i % 200) as f64) - 100.0) * 1e-5)
        .collect();
    ...
    println!(
        "[FUSED(par)] prod_dim(dim={dim}) fwd+bwd f64 [4096,4096]: {best:.2} ms | grad_fp=0x{fp:016x} grad_sum={gsum:.17e}"
    );
    println!(
