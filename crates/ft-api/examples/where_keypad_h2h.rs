//! where(key_pad_mask[B,1,1,S], scores[B,H,S,S], -inf) — the KEY-PADDING attention mask written as
//! `where` (scalar -inf branch, batch varies / heads+query broadcast — MIDDLE/leading, NOT a trailing
//! tile). Fused general-broadcast path (try_where_one_scalar via broadcast_offset_plan) vs torch. Set
//! FT_ORIG=1 (fast path disabled + rebuilt) to time the fallthrough (broadcast_to x3 + tape where).
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // ---- parity: small, mask [b,1,1,s] into scores [b,h,s,s], y = scalar ----
    ...
    let py_s = format!(
        r#"
import torch
m=torch.tensor({mask_s:?},dtype=torch.float64).reshape({bb},1,1,{ss})
x=torch.tensor({sc:?},dtype=torch.float64).reshape({bb},{hh},{ss},{ss})
o=torch.where(m!=0,x,torch.tensor({neg},dtype=torch.float64)).reshape(-1)
...
"#
    ...
    let yscal = s.tensor_variable(vec![neg], vec![1], false)?;
    let o = s.tensor_where(m, x, yscal)?;
    ...
    for _ in 0..9 {
        let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
        let m = s.tensor_variable(md.clone(), vec![b_, 1, 1, s_], false).unwrap();
        let x = s.tensor_variable(xd.clone(), vec![b_, h_, s_, s_], false).unwrap();
        let yscal = s.tensor_variable(vec![neg], vec![1], false).unwrap();
        let t = Instant::now();
        let _ = s.tensor_where(m, x, yscal).unwrap();
...
Y=torch.tensor({neg},dtype=torch.float64)
def t(fn,reps=9):
    for _ in range(2): fn()
    ts=[]
    for _ in range(reps): st=time.perf_counter(); fn(); ts.append((time.perf_counter()-st)*1e3)
    return min(ts)
print("PT w %.4f"%t(lambda:torch.where(m,x,Y)))
"#
    );
    let mut ch = Command::new(&python).arg("-").stdin(Stdio::piped()).stdout(Stdio::piped()).spawn()?;
    ch.stdin.as_mut().unwrap().write_all(py_b.as_bytes())?;
    let pt = String::from_utf8_lossy(&ch.wait_with_output()?.stdout).to_string();
    let ptw = pt.lines().find_map(|l| l.strip_prefix("PT w ")).and_then(|s| s.trim().parse::<f64>().ok()).unwrap_or(f64::NAN);
    let v = |ft: f64, p: f64| if p >= ft { format!("FT {:.2}x FASTER", p / ft) } else { format!("FT {:.2}x SLOWER", ft / p) };
    println!("  where(kpad,x,-inf) {label} {best:.3}ms  PT {ptw:.3}ms  => {}", v(best, ptw));
    Ok(())
}
