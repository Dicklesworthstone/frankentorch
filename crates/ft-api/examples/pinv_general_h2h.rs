...
fn main() {
    for (bb, k) in [(100000usize, 4usize), (20000, 16), (4000, 32)] {
        ...
        for (x, value) in data.iter_mut().enumerate().take(bb * k * k) {
            *value = (((x * 2654435761usize) % 9973) as f64) * 0.001 - 5.0;
        }
        for b in 0..bb {
            for d in 0..k {
                data[b * k * k + d * k + d] += (2 * k) as f64;
            }
        } // diag-dominant -> full rank
        let mut best = f64::INFINITY;
        let mut iderr = 0.0;
        for _ in 0..5 {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let x = s
                .tensor_variable(data.clone(), vec![bb, k, k], false)
                .unwrap();
            let t = Instant::now();
            let pinv = s.tensor_linalg_pinv(x).unwrap();
            ...
            if el < best {
                ...
                let xx = s
                    .tensor_variable(data.clone(), vec![bb, k, k], false)
                    .unwrap();
                let prod = s.tensor_matmul(xx, pinv).unwrap();
                let pv = s.tensor_values(prod).unwrap();
                let mut e = 0.0;
                for b in 0..bb.min(2000) {
                    for i in 0..k {
                        for j in 0..k {
                            let w = if i == j { 1.0 } else { 0.0 };
                            e += (pv[b * k * k + i * k + j] - w).abs();
                        }
                    }
                }
