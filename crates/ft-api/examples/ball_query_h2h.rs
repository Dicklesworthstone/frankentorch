...
fn main() {
    ...
    for _ in 0..7 {
        ...
        let pt = s
            .tensor_variable(pts.clone(), vec![batch, n_points, 3], false)
            .unwrap();
        let qt = s
            .tensor_variable(qrs.clone(), vec![batch, n_queries, 3], false)
            .unwrap();
        let t0 = Instant::now();
    ...
    println!(
        "[ball_query B={batch} N={n_points} M={n_queries} ms_samples={max_samples} r={radius}] threads={threads}: {best:.2} ms"
    );
}
