...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let a: Vec<f64> = (0..R * C).map(|i| 0.05 + ((i % 17) as f64) * 0.05).collect();
    let b: Vec<f64> = (0..R * C).map(|i| 0.05 + ((i % 13) as f64) * 0.07).collect();
    ...
    let ops: Vec<(&str, Op)> = vec![
        ...
        ("l1", |s, x, y| { let _ = s.tensor_l1_loss(x, y, "none"); }),
        ("smooth_l1", |s, x, y| { let _ = s.tensor_smooth_l1_loss(x, y, "none", 1.0); }),
        ("huber", |s, x, y| { let _ = s.tensor_huber_loss(x, y, "none", 1.0); }),
        ("soft_margin", |s, x, y| { let _ = s.tensor_soft_margin_loss(x, y, "none"); }),
        ("bce", |s, x, y| { let _ = s.tensor_binary_cross_entropy(x, y, "none"); }),
    ];
