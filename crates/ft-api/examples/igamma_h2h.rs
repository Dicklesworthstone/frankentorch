...
fn main() {
    ...
    for which in ["igamma", "igammac"] {
        ...
        let ta = s.tensor_variable(a.clone(), vec![rows, cols], false).unwrap();
        let tx = s.tensor_variable(x.clone(), vec![rows, cols], false).unwrap();
        let call = |s: &mut FrankenTorchSession, ta, tx| if which == "igamma" { s.tensor_igamma(ta, tx).unwrap() } else { s.tensor_igammac(ta, tx).unwrap() };
        let _ = call(&mut s, ta, tx);
        let t = bench(9, || { let t0 = Instant::now(); let o = call(&mut s, ta, tx); let e = t0.elapsed().as_micros(); std::hint::black_box(o); e });
