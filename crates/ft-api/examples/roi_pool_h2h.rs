...
fn main() {
    ...
    // roi_pool: [1,256,50,50], 512 ROIs, 7x7 out.
    {
        ...
        let feat: Vec<f64> = (0..n * c * h * w)
            .map(|i| {
                ((i as u64).wrapping_mul(0x9e3779b97f4a7c15) >> 40) as f64 / (1u64 << 24) as f64
            })
            .collect();
        ...
        println!(
            "[roi_pool C={c} H={h} W={w} K={k} out={out_h}x{out_w}] threads={threads}: {best:.2} ms"
        );
    }
    ...
    // ps_roi_pool: [1, 21*49, 50, 50], 512 ROIs, output_size 7.
    {
        ...
        println!(
            "[ps_roi_pool C={c} K={k} osz={output_size} ncls={num_classes}] threads={threads}: {best:.2} ms"
        );
    }
}
