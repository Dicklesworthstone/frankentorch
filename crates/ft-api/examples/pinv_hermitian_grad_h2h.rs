...
// Symmetric positive-definite planes (diagonally dominant symmetric).
fn fill(batch: usize, n: usize) -> Vec<f64> {
    ...
    for plane in 0..batch {
        ...
        // symmetrize + SPD diagonal
        let mut sym = vec![0.0_f64; n * n];
        for r in 0..n {
            for c in 0..n {
                sym[r * n + c] = 0.5 * (a[base + r * n + c] + a[base + c * n + r]);
            }
            sym[r * n + r] += 3.0 + r as f64;
        }
        a[base..base + n * n].copy_from_slice(&sym);
    }
    a
}
...
fn run_ft(batch: usize, n: usize) -> Result<(f64, f64), Box<dyn Error>> {
    ...
    for _ in 0..5 {
        ...
        let y = s.tensor_linalg_pinv_hermitian(a).map_err(boxed)?;
...
off = ((((p+1)*(rows+2)*(cols+3))%17).to(torch.float64)-8.0)*0.01
sym = 0.5*(off + off.transpose(-1,-2))
A = sym + torch.diag_embed(3.0 + torch.arange(N,dtype=torch.float64).reshape(1,N))
def step():
    Ar=A.clone().requires_grad_(True)
    Y=torch.linalg.pinv(Ar, hermitian=True)
    (Y*Y).sum().backward()
    return Ar.grad
