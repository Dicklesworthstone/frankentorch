...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mat: Vec<f64> = (0..R * C)
        .map(|i| ((i.wrapping_mul(2654435761) % 9973) as f64) - 4986.0)
        .collect();
...
"#;
    ...
    report(
        "softmax",
        timed(&mat, |s, x| {
            let _ = s.tensor_softmax(x, 1);
        }),
    );
    report(
        "log_softmax",
        timed(&mat, |s, x| {
            let _ = s.tensor_log_softmax(x, 1);
        }),
    );
    report(
        "layer_norm",
        timed(&mat, |s, x| {
            let _ = s.functional_layer_norm(x, vec![C], None, None, 1e-5);
        }),
    );
    report(
        "rms_norm",
        timed(&mat, |s, x| {
            let _ = s.functional_rms_norm(x, vec![C], None, 1e-5);
        }),
    );
    Ok(())
}
