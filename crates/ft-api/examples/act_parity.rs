...
"#,
    ...
    let pr6 = parse(lines[0]);
    let phs = parse(lines[1]);
    ...
    let tim = |name: &str, f: &dyn Fn(&mut FrankenTorchSession, ft_autograd::TensorNodeId)| {
        ...
        for _ in 0..7 {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let x = s
                .tensor_variable_f32(big.clone(), vec![R, C], false)
                .unwrap();
            let t = Instant::now();
            f(&mut s, x);
