...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut a: Vec<f32> = (0..r*c).map(|i| (((i*37)%613) as f32 - 306.0) * 0.013).collect();
    // seed specials: a NaN in one column + one row, ±0 ties
    a[5*c + 7] = f32::NAN;
    a[10*c + 11] = f32::from_bits(0x7fc0_abcd);
    a[0] = -0.0; a[1] = 0.0; a[c] = 0.0; a[c+1] = -0.0;
    a[20*c + 20] = f32::INFINITY; a[21*c + 21] = f32::NEG_INFINITY;

    let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
    let x = s.tensor_variable_f32(a.clone(), vec![r, c], false)?;
    let n0 = s.tensor_amax(x, 0)?; let amax0 = s.tensor_values_f32(n0)?;
    let n1 = s.tensor_amax(x, 1)?; let amax1 = s.tensor_values_f32(n1)?;
    let x2 = s.tensor_variable_f32(a.clone(), vec![r, c], false)?;
    let m0 = s.tensor_amin(x2, 0)?; let amin0 = s.tensor_values_f32(m0)?;
    let m1 = s.tensor_amin(x2, 1)?; let amin1 = s.tensor_values_f32(m1)?;
