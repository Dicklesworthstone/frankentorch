...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ops: &[(&str, u8)] = &[
        ("add", 0), ("hypot", 1), ("copysign", 2), ("xlogy", 3), ("logaddexp", 4),
        ("nextafter", 5), ("heaviside", 6), ("float_power", 7),
    ];
    let bench = |w: u8| {
        ...
        for _ in 0..7 {
            ...
            match w {
                ...
                1 => { let _ = s.tensor_hypot(x, y); }
                2 => { let _ = s.tensor_copysign(x, y); }
                3 => { let _ = s.tensor_xlogy(x, y); }
                4 => { let _ = s.tensor_logaddexp(x, y); }
                5 => { let _ = s.tensor_nextafter(x, y); }
                6 => { let _ = s.tensor_heaviside(x, y); }
                _ => { let _ = s.tensor_float_power(x, 2.5); }
            }
...
"#);
    ...
    for &(lbl, w) in ops {
        let ft = bench(w);
        println!("  {lbl:<12} FT {ft:8.3}  PT {:8.3}  => {}", g(lbl), vrb(ft, g(lbl)));
    }
    Ok(())
}
