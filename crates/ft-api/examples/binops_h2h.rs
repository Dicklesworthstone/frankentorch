//! Core binary arithmetic + scalar op scan FT vs PyTorch ([4000,4000] f64 no-grad).
//! `cat` is a landed-win ANCHOR.
//! Run: PYTORCH_PYTHON=/path/to/python cargo run --release -p ft-api --example binops_h2h
...
for name,fn in [("cat_anchor",lambda:torch.cat([x,x],1)),
                ("add",lambda:x+y),("mul",lambda:x*y),("div",lambda:x/y),
