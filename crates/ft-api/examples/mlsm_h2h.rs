...
fn main() {
    ...
    let input: Vec<f64> = (0..n * c)
        .map(|i| {
            ((i as u64).wrapping_mul(0x9e3779b97f4a7c15) >> 40) as f64 / (1u64 << 24) as f64 * 6.0
                - 3.0
        })
        .collect();
    ...
    for _ in 0..7 {
        ...
        let t0 = Instant::now();
        let loss = s
            .tensor_multilabel_soft_margin_loss(inp, tgt, None, "mean")
            .unwrap();
        let t1 = Instant::now();
    ...
    println!(
        "[mlsm n={n} c={c}] threads={threads}: fwd {bf:.2} bwd {bb:.2} fwd+bwd {:.2} ms",
        bf + bb
    );
}
