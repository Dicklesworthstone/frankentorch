...
fn main() {
    ...
    // f64
    {
        ...
        let scores = ses
            .tensor_variable(scores_f64.clone(), vec![b, h, s, s], false)
            .unwrap();
        let bias = ses
            .tensor_variable(bias_f64.clone(), vec![1, 1, s, s], false)
            .unwrap();
        // warm
        ...
        println!(
            "[{tag}] add f64 [16,16,128,128]+[1,1,128,128]: {:.2} ms",
            t as f64 / 1000.0
        );
    }
    // f32
    {
        let mut ses = FrankenTorchSession::new(ExecutionMode::Strict);
        let scores = ses
            .tensor_variable_f32(scores_f32.clone(), vec![b, h, s, s], false)
            .unwrap();
        let bias = ses
            .tensor_variable_f32(bias_f32.clone(), vec![1, 1, s, s], false)
            .unwrap();
        let _ = ses.tensor_add(scores, bias).unwrap();
        let t = bench(9, || {
            let t0 = Instant::now();
        ...
        println!(
            "[{tag}] add f32 [16,16,128,128]+[1,1,128,128]: {:.2} ms",
            t as f64 / 1000.0
        );
    }
    // key-padding [B,1,1,S] add f32
    {
        ...
        let scores = ses
            ...
            .unwrap();
        let bias = ses
            .tensor_variable_f32(bias_kp.clone(), vec![b, 1, 1, s], false)
            .unwrap();
        let _ = ses.tensor_add(scores, bias).unwrap();
        let t = bench(9, || {
            let t0 = Instant::now();
        ...
        println!(
            "[{tag}] add f32 keypad [16,16,128,128]+[16,1,1,128]: {:.2} ms",
            t as f64 / 1000.0
        );
    }
}
