...
"#;
    ...
    rep("add_anchor", t1(&a, vec![R,C], |s,x| { let _=s.tensor_add(x,x); }));
    { let mut best=f64::INFINITY; for _ in 0..7 { let mut s=FrankenTorchSession::new(ExecutionMode::Strict);
        ...
        let mk=s.tensor_variable_f32(mask.clone(),vec![R,C],false).unwrap();
        let t=Instant::now(); let _=s.tensor_masked_fill(x,mk,-1.5); let e=t.elapsed().as_secs_f64()*1e3; if e<best{best=e;} } rep("masked_fill",best); }
    rep("pad", t1(&a, vec![R,C], |s,x| { let _=s.tensor_pad(x, &[8,8,8,8], 2.0); }));
    rep("rot90", t1(&a, vec![R,C], |s,x| { let _=s.tensor_rot90(x, 1, [0,1]); }));
    rep("diag_embed", t1(&de, vec![200,500], |s,x| { let _=s.tensor_diag_embed(x, 0); }));
    Ok(())
}
