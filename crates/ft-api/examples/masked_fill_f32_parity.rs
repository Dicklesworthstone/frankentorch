...
"#, bits, mbits);
    ...
    let o2=ch.wait_with_output()?; let out=String::from_utf8_lossy(&o2.stdout);
    let pt: Vec<u32> = out.lines().next().unwrap_or("").split_whitespace().filter_map(|t|t.parse().ok()).collect();
    let mut mm = if dt != DType::F32 {println!("DTYPE MISMATCH"); 1} else {0};
    for i in 0..ft.len() {
        let fb=ft[i].to_bits(); let pb=pt[i];
        let eq = fb==pb || (f32::from_bits(fb).is_nan() && f32::from_bits(pb).is_nan());
        if !eq { mm+=1; if mm<=3 { println!("idx {i}: ft={} pt={}", f32::from_bits(fb), f32::from_bits(pb)); } }
    }
