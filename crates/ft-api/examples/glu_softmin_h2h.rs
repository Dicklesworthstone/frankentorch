...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let n_glu = b * s * 2 * d;
    let n_sm = b * s * d;
    let glu_x: Vec<f64> = (0..n_glu).map(|i| ((i % 991) as f64) * 0.01 - 5.0).collect();
    ...
    let run = |which: &str| -> f64 {
        ...
        for _ in 0..7 {
            ...
            let (v, shp) = if which == "glu" { (&glu_x, vec![b, s, 2 * d]) } else { (&sm_x, vec![b, s, d]) };
            let x = sess.tensor_variable(v.clone(), shp, false).unwrap();
            let t = Instant::now();
            let _ = if which == "glu" { sess.tensor_glu(x, 2).unwrap() } else { sess.tensor_softmin(x, 2).unwrap() };
...
"#
    ...
    println!("  glu     FT {fg:.3}ms  PT {:.3}ms => {}", g("glu"), v(fg, g("glu")));
    println!("  softmin FT {fs:.3}ms  PT {:.3}ms => {}", g("softmin"), v(fs, g("softmin")));
    Ok(())
}
