...
fn old_slice_scatter(input: &[f64], src: &[f64], row_len: usize, start: usize, slice_len: usize) -> Vec<f64> {
...
fn main() {
    println!("tensor_slice_scatter f64 dim0, min-9:  OLD=to_vec + serial  NEW=borrow + parallel copy");
    // (label, rows, cols, slice rows replaced)
    let cases: [(&str, usize, usize, usize); 3] =
        [("8000x2000 slice256", 8000, 2000, 256), ("4000x4000 slice128", 4000, 4000, 128), ("16000x1000 slice512", 16000, 1000, 512)];
    for (label, rows, cols, slice_len) in cases {
        let numel = rows * cols;
        let input: Vec<f64> = (0..numel).map(|i| (i % 251) as f64 * 0.5).collect();
        let start = rows / 4;
        let src: Vec<f64> = (0..slice_len * cols).map(|i| (i % 97) as f64 + 1000.0).collect();
        ...
        let st = sess.tensor_variable(src.clone(), vec![slice_len, cols], false).unwrap();
        let out = sess
            .tensor_slice_scatter(it, st, 0, Some(start as i64), Some((start + slice_len) as i64), 1)
            .unwrap();
        ...
        let new_ms = bench(|| {
            sess.tensor_slice_scatter(it, st, 0, Some(start as i64), Some((start + slice_len) as i64), 1)
                .unwrap()
                .0
        });
        println!(
