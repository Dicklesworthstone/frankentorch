...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let a: Vec<f32> = (0..n).map(|i| {
        ...
        else { 0.9 + ((i % 4001) as f32 / 4000.0) * 0.2 }
    }).collect();
    ...
    let finite: Vec<f32> = a.iter().map(|&x| if x.is_infinite() { 1.0 } else { x }).collect();
...
"#);
    ...
    let g = |k: &str| out.lines().find_map(|l| l.strip_prefix(k)?.trim().parse::<f64>().ok()).unwrap_or(f64::NAN);
    let (tv, td) = (g("REFVAR"), g("REFSTD"));
    let rel = |a: f64, b: f64| (a - b).abs() / b.abs().max(1e-30);
    println!("nanvar  FT {ft_var:.10e}  torch {tv:.10e}  rel_err {:.3e}", rel(ft_var, tv));
    println!("nanstd  FT {ft_std:.10e}  torch {td:.10e}  rel_err {:.3e}", rel(ft_std, td));
    Ok(())
}
