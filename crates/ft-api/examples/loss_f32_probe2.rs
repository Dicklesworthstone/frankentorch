...
fn main() {
    ...
    let z1 = f32t(&mut s, vec![0.5, 0.3, 0.2, 0.1, 0.4, 0.6, 0.2, 0.8, 0.1, 0.3, 0.5, 0.7], vec![4, 3]);
    let z2 = f32t(&mut s, vec![0.4, 0.2, 0.3, 0.2, 0.5, 0.5, 0.3, 0.7, 0.2, 0.4, 0.4, 0.6], vec![4, 3]);
    ...
    probe!("arcface_loss", s.arcface_loss(feat, weight, labels, 30.0, 0.5));
    probe!("cosface_loss", s.cosface_loss(feat, weight, labels, 30.0, 0.35));
    probe!("barlow_twins_loss", s.barlow_twins_loss(z1, z2, 0.005));
    probe!("vicreg_loss", s.vicreg_loss(z1, z2, 25.0, 25.0, 1.0));
    probe!("multi_margin_loss", s.multi_margin_loss(mm_in, mm_tgt, 1.0, 1.0));
