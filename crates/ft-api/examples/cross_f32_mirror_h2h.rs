...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bench = || {
        ...
        for _ in 0..5 {
            ...
            let a = s
                .tensor_variable_f32(mka.clone(), vec![rows, 3], false)
                .unwrap();
            let b = s
                .tensor_variable_f32(mkb.clone(), vec![rows, 3], false)
                .unwrap();
            let t = Instant::now();
...
"#
    ...
    println!(
        "cross f32 [{rows}x3]: FT {ft:8.3}ms torch {pt:8.3}ms => {vrb} | dtype={dt:?} exact={exact}/{}",
        fv.len().min(tv.len())
    );
    Ok(())
}
