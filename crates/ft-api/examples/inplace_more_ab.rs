...
fn old_copysign(t: &[f64], o: &[f64]) -> Vec<f64> {
    let tv = t.to_vec();
    let ov = o.to_vec();
    tv.iter().zip(ov.iter()).map(|(&m, &s)| m.copysign(s)).collect()
}
fn old_remainder(t: &[f64], o: &[f64]) -> Vec<f64> {
    let tv = t.to_vec();
    let ov = o.to_vec();
    tv.iter().zip(ov.iter()).map(|(&a, &b)| a - (a / b).floor() * b).collect()
}
fn old_xlogy(t: &[f64], o: &[f64]) -> Vec<f64> {
    ...
    tv.iter()
        .zip(ov.iter())
        .map(|(&x, &y)| if x == 0.0 && !y.is_nan() { 0.0 } else { x * y.ln() })
        .collect()
}
...
fn main() {
    println!("in-place copysign_/remainder_/xlogy_ f64, min-9:  OLD=clone-both+serial  NEW=borrow-both+parallel");
    ...
    let cn = bench(|| { s.tensor_copysign_(at, bt).unwrap(); s.tensor_values(at).unwrap().len() });
    ...
    let rn = bench(|| { s2.tensor_remainder_(at2, bt2).unwrap(); s2.tensor_values(at2).unwrap().len() });
    ...
    let xn = bench(|| { s3.tensor_xlogy_(at3, bt3).unwrap(); s3.tensor_values(at3).unwrap().len() });

    println!("  n={} (512MB)  copysign_ {:.2}x (bm={})  remainder_ {:.2}x (bm={})  xlogy_ {:.2}x (bm={})",
        n, co / cn, bm_c, ro / rn, bm_r, xo / xn, bm_x);
}
