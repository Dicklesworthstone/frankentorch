...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let nn = 4096usize;
    let input: Vec<f32> = (0..nn * nn).map(|i| ((i % 9973) as f32 - 5000.0) * 0.001).collect();
    let src: Vec<f32> = (0..nn).map(|i| ((i % 7919) as f32) * 0.001 + 0.5).collect();
    let bench = || {
        let mut best = f64::INFINITY;
        for _ in 0..5 {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let xi = s.tensor_variable_f32(input.clone(), vec![nn, nn], false).unwrap();
    ...
    let o = match s.tensor_diagonal_scatter(xi, sr, 0) { Ok(o) => o, Err(e) => { println!("diagonal_scatter f32: FT ERROR {e:?}"); return Ok(()); } };
...
"#);
    ...
    let pt = out.lines().find_map(|l| { let mut it = l.strip_prefix("PT ")?.split_whitespace(); if it.next()? == "ds" { it.next()?.parse::<f64>().ok() } else { None } }).unwrap_or(f64::NAN);
    let ft = bench();
    ...
    println!("diagonal_scatter f32 [{nn}x{nn}]: FT {ft:8.3}ms torch {pt:8.3}ms => {vrb} | dtype={dt:?} exact={exact}/{}", fv.len().min(tv.len()));
    Ok(())
}
