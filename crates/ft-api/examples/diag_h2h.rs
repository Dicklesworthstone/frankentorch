...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data: Vec<f32> = (0..nn * nn)
        .map(|i| ((i % 9973) as f32 / 1000.0) - 5.0)
        .collect();
    let bench = |w: u8| {
        ...
        for _ in 0..7 {
            ...
            let x = s
                .tensor_variable_f32(data.clone(), vec![nn, nn], false)
                .unwrap();
            let t = Instant::now();
            if w == 0 {
                let _ = s.tensor_add(x, x);
            } else {
                let _ = s.tensor_diagonal(x, 0);
            }
...
"#
    ...
    for (&f, &t) in fv.iter().take(tv.len()).zip(tv.iter()) {
        if f.to_bits() == t.to_bits() {
            exact += 1;
        }
    }
    println!(
        "correctness: diagonal dtype={dt:?} len={} bit_exact={exact}/{}",
        fv.len(),
        tv.len()
    );
    Ok(())
