...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bench = |w: u8| {
        ...
        for _ in 0..7 {
            ...
            match w {
                0 => {
                    let _ = s.tensor_add(x, t);
                }
                1 => {
                    let _ = s.tensor_bce_with_logits_loss(x, t, "mean");
                }
                2 => {
                    let _ = s.tensor_poisson_nll_loss(x, t, true, false, 1e-8, "mean");
                }
                3 => {
                    let _ = s.tensor_bce_with_logits_pos_weight(x, t, 2.5, "mean");
                }
                _ => {
                    let v = s.tensor_variable_f32(varv.clone(), vec![n], false).unwrap();
                    let _ = s.tensor_gaussian_nll_loss(x, t, v, "mean", false);
                }
            }
...
"#
    ...
    for (lbl, w, key) in [
        ("add", 0u8, "add"),
        ("bce_logits", 1, "bce_logits"),
        ("poisson", 2, "poisson"),
        ("bce_pw", 3, "bce_pw"),
        ("gauss_nll", 4, "gauss_nll"),
    ] {
        let ft = bench(w);
        println!(
