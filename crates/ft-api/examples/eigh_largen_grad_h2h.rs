...
// Symmetric planes (M + Mᵀ + 2I), deterministic.
...
fn run_ft(batch: usize, n: usize) -> Result<f64, Box<dyn Error>> {
    ...
    for _ in 0..3 {
        ...
        let loss = s.tensor_add(sw, sv).map_err(boxed)?;
...
V=((((p+1)*(rows+2)*(cols+3))%19).to(torch.float64)-9.0)*0.02
A=V+V.transpose(-1,-2)+torch.eye(N,dtype=torch.float64).unsqueeze(0)*2.0
def step():
    Ar=A.clone().requires_grad_(True); w,Ve=torch.linalg.eigh(Ar); (w.sum()+Ve.sum()).backward(); return Ar.grad
