...
"#
    ...
    for _ in 0..9 {
        let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
        let m = s
            .tensor_variable(md.clone(), vec![1, 1, s_, s_], false)
            .unwrap();
        let a = s
            .tensor_variable(ad.clone(), vec![b_, h_, s_, s_], false)
            .unwrap();
        let b = s
            .tensor_variable(bd.clone(), vec![b_, h_, s_, s_], false)
            .unwrap();
        let t = Instant::now();
...
"#
    ...
    let ptw = pt
        .lines()
        .find_map(|l| l.strip_prefix("PT where "))
    ...
    println!(
        "  where(mask,a,b) {label} {best:.3}ms  PT {ptw:.3}ms  => {}",
        v(best, ptw)
    );
    Ok(())
}
