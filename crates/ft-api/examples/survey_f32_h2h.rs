...
fn t2<F: Fn(&mut FrankenTorchSession, TensorNodeId, TensorNodeId)>(
    ...
    for _ in 0..7 {
        ...
        let x = s
            ...
            .unwrap();
        let y = s
            .tensor_variable_f32(b2.to_vec(), vec![R, C], false)
            .unwrap();
        let t = Instant::now();
        f(&mut s, x, y);
        let e = t.elapsed().as_secs_f64() * 1e3;
...
"#;
    ...
    rep("where", {
        ...
        for _ in 0..7 {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let cn = s
                .tensor_variable_f32(cc.clone(), vec![R, C], false)
                .unwrap();
            let xn = s.tensor_variable_f32(a.clone(), vec![R, C], false).unwrap();
            let yn = s
                .tensor_variable_f32(bb.clone(), vec![R, C], false)
                .unwrap();
            let t = Instant::now();
            let _ = s.tensor_where(cn, xn, yn);
            let e = t.elapsed().as_secs_f64() * 1e3;
            if e < bst {
                bst = e;
            }
    ...
    rep("addcmul", {
        ...
        for _ in 0..7 {
            ...
            let yn = s
                ...
                .unwrap();
            let t = Instant::now();
            let _ = s.tensor_addcmul(xn, yn, xn, 0.5);
            let e = t.elapsed().as_secs_f64() * 1e3;
            if e < bst {
                bst = e;
            }
