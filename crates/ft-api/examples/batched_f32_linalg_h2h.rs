    ...
    for (bb, k) in [(100000usize, 4usize), (20000, 16)] {
        ...
        let run = |op: &str| -> (f64, f64) {
            ...
            for _ in 0..5 {
                ...
                let c = match op {
                    "svd" => {
                        let (_u, sv, _v) = s.tensor_linalg_svd(x, true).unwrap();
                        let e = t.elapsed().as_secs_f64() * 1e3;
                        let c = s.tensor_values_lossy_f64(sv).unwrap().iter().sum();
                        (e, c)
                    }
                    "svdvals" => {
                    ...
                    "eigvals" => {
                        let w = s.tensor_linalg_eigvals(x).unwrap();
                        let e = t.elapsed().as_secs_f64() * 1e3;
                        let c = s
                            .tensor_values_lossy_f64(w)
                            .unwrap()
                            .iter()
                            .step_by(2)
                            .sum();
...
"#
            ...
            let Ok(o) = Command::new(&python).arg("-c").arg(&pysrc).output() else {
                println!();
                continue;
            };
            if !o.status.success() {
            ...
            if let (Some(p), Some(pc)) = (g("MS "), g("CHK ")) {
                let rel = (chk - pc).abs() / (pc.abs() + 1e-6);
                let rr = p / ft_ms;
                println!(
                    " | torch {p:.1}ms chk {pc:.4e} | {} | {}",
                    if rel < 1e-3 { "OK" } else { "DIFF" },
                    if rr >= 1.0 {
                        format!("FT {rr:.2}x FASTER")
                    } else {
                        format!("{:.2}x slower", 1.0 / rr)
                    }
                );
            }
        }
