...
"#,
    ...
    println!(
        "parity f32: dtype={dt:?} (now WORKS, was ERROR) {mm}/{} per-element mismatches",
        pv.len()
    );
    ...
    let xb: Vec<f32> = (0..n)
        ...
        .collect();
    let tb: Vec<f32> = (0..n)
        .map(|i| ((i % 7919) as f32 - 4000.0) * 0.0011)
        .collect();
    let tt = |which: u8| {
        let mut b = f64::INFINITY;
        for _ in 0..9 {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let x = s.tensor_variable_f32(xb.clone(), vec![n], false).unwrap();
            let t = s.tensor_variable_f32(tb.clone(), vec![n], false).unwrap();
            let ti = Instant::now();
            let _ = if which == 0 {
                s.tensor_relu(x)
            } else {
                s.tensor_smooth_l1_loss(x, t, "mean", beta)
            };
