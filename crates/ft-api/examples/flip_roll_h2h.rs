//! Multi-dim flip / roll FT vs PyTorch ([4000,4000] f64 no-grad). flip([0,1]) and 2D roll
//! fall through to the tape per-element division-unravel (single-dim already fast-pathed).
//! `cat` is a landed-win ANCHOR for worker health.
//! Run: PYTORCH_PYTHON=/path/to/python cargo run --release -p ft-api --example flip_roll_h2h
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ops: Vec<(&str, fn(&mut FrankenTorchSession, ft_autograd::TensorNodeId))> = vec![
        ...
        ("flip_both", |s, x| { let _ = s.tensor_flip(x, &[0, 1]); }),
        ("flip_last", |s, x| { let _ = s.tensor_flip(x, &[1]); }),
        ("flip_first", |s, x| { let _ = s.tensor_flip(x, &[0]); }),
        ("roll_last", |s, x| { let _ = s.tensor_roll(x, 13, 1); }),
    ];
...
for name,fn in [("cat_anchor",lambda:torch.cat([x,x],1)),
                ("flip_both",lambda:torch.flip(x,[0,1])),
                ("flip_last",lambda:torch.flip(x,[1])),
                ("flip_first",lambda:torch.flip(x,[0])),
                ("roll_last",lambda:torch.roll(x,13,1))]:
