...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let n_relu = s.tensor_relu(xa)?;
    let relu = s.tensor_values_f32(n_relu)?;
    let n_neg = s.tensor_neg(xa)?;
    let neg = s.tensor_values_f32(n_neg)?;
    let n_abs = s.tensor_abs(xa)?;
    let abs = s.tensor_values_f32(n_abs)?;
    let n_sqrt = s.tensor_sqrt(xp)?;
    let sqrt = s.tensor_values_f32(n_sqrt)?;
    let n_rcp = s.tensor_reciprocal(xp)?;
    let rcp = s.tensor_values_f32(n_rcp)?;
