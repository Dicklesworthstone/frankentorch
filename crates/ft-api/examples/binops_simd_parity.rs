...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let na = s.tensor_add(xa, xb)?;
    let add = s.tensor_values_f32(na)?;
    let ns = s.tensor_sub(xa, xb)?;
    let sub = s.tensor_values_f32(ns)?;
    let nm = s.tensor_mul(xa, xb)?;
    let mul = s.tensor_values_f32(nm)?;
    let nd = s.tensor_div(xa, xb)?;
    let div = s.tensor_values_f32(nd)?;
