...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let run = |which: &str| -> f64 {
        ...
        for _ in 0..7 {
            ...
            let t = Instant::now();
            let _ = if which == "shuffle" {
                s.tensor_pixel_shuffle(x, r).unwrap()
            } else {
                s.tensor_pixel_unshuffle(x, r).unwrap()
            };
...
"#
    ...
    println!(
        "  pixel_shuffle   FT {fsh:.3}ms  PT {:.3}ms => {}",
        g("shuffle"),
        v(fsh, g("shuffle"))
    );
    println!(
        "  pixel_unshuffle FT {fun:.3}ms  PT {:.3}ms => {}",
        g("unshuffle"),
        v(fun, g("unshuffle"))
    );
    Ok(())
