...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repstr: String = reps.iter().map(|r| r.to_string()).collect::<Vec<_>>().join(",");
    let py = format!(r#"
import time,torch
...
"#);
    ...
    let pt = out.lines().find_map(|l| { let mut it = l.strip_prefix("PT ")?.split_whitespace(); if it.next()? == "ri" { it.next()?.parse::<f64>().ok() } else { None } }).unwrap_or(f64::NAN);
    let ft = bench();
    ...
    println!("repeat_interleave_tensor f32 [{rows}x{cols}] reps~1-4 dim=0: FT {ft:8.3}ms torch {pt:8.3}ms => {vrb} | dtype={dt:?} exact={exact}/{}", fv.len().min(tv.len()));
    Ok(())
}
