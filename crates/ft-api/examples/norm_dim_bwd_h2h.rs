...
// Head-to-head timing + grad fingerprint for the fused norm_dim backward
...
fn main() {
    ...
    let n = rows * cols;
    let dim = 1usize; // reduce over columns -> output shape [rows]
    ...
    for p in [2.0_f64, 3.0] {
        ...
        for _ in 0..7 {
            ...
            let loss = s.tensor_sum(nrm).unwrap();
        ...
        println!(
            "[FUSED(par)] norm_dim(p={p},dim={dim}) fwd+bwd f64 [4096,4096]: {best:.2} ms | grad_fp=0x{fp:016x} grad_sum={gsum:.17e}"
        );
        println!(
