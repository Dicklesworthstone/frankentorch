...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let small: Vec<f64> = vec![0.0, -0.0, 0.5, 0.50001, 0.49999, -0.5, 1.0, -1.0, 2.0, -2.0, 0.3];
    let py_s = format!(
        r#"
import torch
...
"#,
    ...
    let op = |which: u8| {
        ...
        for _ in 0..9 {
            ...
            let t = Instant::now();
            let _ = if which == 0 { s.tensor_relu(x) } else { s.tensor_threshold(x, th, va) };
...
"#, n = n, th = th, va = va);
    ...
    println!("  relu_anchor  FT {ft_r:.3} PT {pr:.3}  => FT {:.2}x {}", (pr/ft_r).max(ft_r/pr), if pr>=ft_r {"FASTER"} else {"SLOWER"});
    println!("  threshold    FT {ft_t:.3} PT {ptv:.3}  => FT {:.2}x {}", (ptv/ft_t).max(ft_t/ptv), if ptv>=ft_t {"FASTER"} else {"SLOWER"});
    Ok(())
}
