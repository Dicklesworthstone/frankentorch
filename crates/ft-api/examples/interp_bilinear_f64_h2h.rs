...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let x: Vec<f64> = (0..numel).map(|i| ((i % 9973) as f64 - 5000.0) * 0.01).collect();
    let bench = || {
        let mut best = f64::INFINITY;
        for _ in 0..7 {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let xi = s.tensor_variable(x.clone(), vec![nn, cc, ih, iw], false).unwrap();
            let t = Instant::now();
    ...
    let fv: Vec<f64> = s.tensor_values_lossy_f64(o)?.iter().take(8192).copied().collect();
    let py = format!(r#"
import time,torch
...
"#);
    ...
    let ft = bench(); let pt = g("bi");
    let line = out.lines().find(|l| l.starts_with("REF ")).unwrap_or("");
    let tv: Vec<f64> = line.split_whitespace().skip(1).filter_map(|t| t.parse().ok()).collect();
    let exact = fv.iter().zip(tv.iter()).filter(|(a, b)| a.to_bits() == b.to_bits()).count();
    let close = fv.iter().zip(tv.iter()).filter(|(a, b)| (**a - **b).abs() <= 1e-9 * b.abs().max(1.0)).count();
    ...
    println!("  value: dtype={dt:?} bit_exact={exact}/{} close(1e-9)={close}/{}", fv.len().min(tv.len()), fv.len().min(tv.len()));
    Ok(())
}
