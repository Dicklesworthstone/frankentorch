//! More two-input math op scan FT vs PyTorch ([4000,4000] f64 no-grad). Hunting more of the
//! apply_function unconditional-save + serial-compute pattern. `cat` is a landed-win ANCHOR.
//! Run: PYTORCH_PYTHON=/path/to/python cargo run --release -p ft-api --example elembin2_h2h
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // x positive-ish (for xlogy/float_power), y mixed.
    let a: Vec<f64> = (0..R * C).map(|i| ((i % 17) as f64) * 0.1 + 0.5).collect();
    ...
    let ops: Vec<(&str, Op)> = vec![
        ...
        ("nextafter", |s, x, y| { let _ = s.tensor_nextafter(x, y); }),
        ("heaviside", |s, x, y| { let _ = s.tensor_heaviside(x, y); }),
...
x=((idx%17).double()*0.1+0.5).reshape(R,C)
...
for name,fn in [("cat_anchor",lambda:torch.cat([x,x],1)),
                ("nextafter",lambda:torch.nextafter(x,y)),
                ("heaviside",lambda:torch.heaviside(x,y)),
