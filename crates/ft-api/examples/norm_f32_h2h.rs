...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let n = 16 * 1024 * 1024;
    let inp: Vec<f32> = (0..n).map(|i| ((i % 9973) as f32 - 5000.0) * 0.001).collect();
    let bench = |p: f64| {
        ...
        for _ in 0..7 {
            ...
            if e < best { best = e; val = s.tensor_values_lossy_f64(o).unwrap()[0]; }
        }
        (best, val)
    };
    let (l1, v1) = bench(1.0);
...
"#);
    ...
    for (tag, ft, fv) in [("norm1", l1, v1), ("norm2", l2, v2), ("norm3", l3, v3), ("norminf", linf, vinf), ("norm0", l0, v0)] {
        ...
        println!("{tag:<7} FT {ft:8.4}ms torch {pt:8.4}ms => {vrb} | ftval={fv:.5} ptval={pv:.5} rel={rel:.2e}");
    }
    Ok(())
}
