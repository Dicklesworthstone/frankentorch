...
fn main() {
    ...
    let sm_new = best(
        || {
            ...
            let (x, y) = mk(&mut s, &td);
            let _ = s.tensor_soft_margin_loss(x, y, "none").unwrap();
        },
        7,
    );
    let sm_old = best(
        || {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let (x, y) = mk(&mut s, &td);
            let prod = s.tensor_mul(y, x).unwrap();
            let neg = s.tensor_neg(prod).unwrap();
            let e = s.tensor_exp(neg).unwrap();
            let ones = s.full(vec![R, C], 1.0, false).unwrap();
            let ope = s.tensor_add(ones, e).unwrap();
            let _ = s.tensor_log(ope).unwrap();
        },
        7,
    );
    ...
    let kl_new = best(
        || {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let (x, y) = mk(&mut s, &td);
            let _ = s.tensor_kl_div(x, y, "none", false).unwrap();
        },
        7,
    );
    let kl_old = best(
        || {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let (x, y) = mk(&mut s, &td);
            let logt = s.tensor_log(y).unwrap();
            let diff = s.tensor_sub(logt, x).unwrap();
            let _ = s.tensor_mul(y, diff).unwrap();
        },
        7,
    );
    ...
    let h_new = best(
        || {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let (x, y) = mk(&mut s, &tgn);
            let _ = s.tensor_hinge_embedding_loss(x, y, 1.0, "none").unwrap();
        },
        7,
    );
    let h_old = best(
        || {
            let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
            let (x, y) = mk(&mut s, &tgn);
            let zeros = s.full(vec![R, C], 0.0, false).unwrap();
            let margin_t = s.full(vec![R, C], 1.0, false).unwrap();
            let ones = s.full(vec![R, C], 1.0, false).unwrap();
            let mmi = s.tensor_sub(margin_t, x).unwrap();
            let hinge = s.tensor_maximum(zeros, mmi).unwrap();
            let mp = s.tensor_eq(y, ones).unwrap();
            let _ = s.tensor_where(mp, x, hinge).unwrap();
        },
        7,
    );
    ...
    let cat = best(
        || {
            ...
            let _ = s.tensor_cat(&[x, x], 1).unwrap();
        },
        7,
    );

    println!(
        "same-process A/B (OLD composed vs NEW fast path), threads={}",
        rayon::current_num_threads()
    );
    println!(
        "  soft_margin  OLD {sm_old:8.3}  NEW {sm_new:8.3}  speedup {:.2}x",
        sm_old / sm_new
    );
    println!(
        "  kl_div       OLD {kl_old:8.3}  NEW {kl_new:8.3}  speedup {:.2}x",
        kl_old / kl_new
    );
    println!(
        "  hinge        OLD {h_old:8.3}  NEW {h_new:8.3}  speedup {:.2}x",
        h_old / h_new
    );
