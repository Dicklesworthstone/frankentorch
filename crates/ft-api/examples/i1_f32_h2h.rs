...
fn main() {
    ...
    for which in ["i1", "special_i1"] {
        ...
        let a = s.tensor_variable_f32(data.clone(), vec![rows, cols], false).unwrap();
        let call = |s: &mut FrankenTorchSession, a| if which == "i1" { s.tensor_i1(a).unwrap() } else { s.tensor_special_i1(a).unwrap() };
        let o0 = call(&mut s, a);
        ...
        println!("[FUSED-f32] {which} f32 [4096,4096]: {:.2} ms  dtype={:?}", t as f64 / 1000.0, dt);
    }
}
