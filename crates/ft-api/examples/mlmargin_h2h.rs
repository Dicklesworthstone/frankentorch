...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bench = || {
        ...
        for _ in 0..7 {
            ...
            let x = s
                .tensor_variable_f32(inp.clone(), vec![nn, c], false)
                .unwrap();
...
"#
    ...
    let vrb = if pt >= ft_ms {
        format!("FT {:.2}x FASTER", pt / ft_ms)
    } else {
        format!("FT {:.2}x SLOWER", ft_ms / pt)
    };
