...
"#;
    ...
    for _ in 0..7 {
        ...
        let xn = s
            .tensor_variable_f32(xc.clone(), vec![R, R], false)
            .unwrap();
        let t = Instant::now();
        let _ = s.tensor_cat(&[xn, xn], 1);
    ...
    for _ in 0..7 {
        let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
        let xn = s.tensor_variable_f32(x.clone(), vec![R, R], false).unwrap();
        let t = Instant::now();
        let _ = s.tensor_movedim(xn, 0, 1);
        let e = t.elapsed().as_secs_f64() * 1e3;
        if e < m {
            m = e;
        }
